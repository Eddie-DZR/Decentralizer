# Trademark

“Decentralizer” and the Decentralizer logo are used as project identifiers.

## Open-source licensing
The code and hardware in this repository are licensed as stated in the LICENSE files (MIT / CERN-OHL-S-2.0).  
Trademark guidance is separate from the open-source licenses.

## Allowed use
- You may refer to “Decentralizer” to describe or credit the original project.
- You may link to this repository and use the name in documentation that clearly points to the original project.

## Contact
For questions or permission requests: contact@decentralizer.org



## Naming
We kindly ask that all forks, modifications, and derivatives of this project continue to use the name **Decentralizer**.  
This helps preserve the identity and spirit of the project.
Thank you for honoring the vision behind this work.


<img width="512" height="128" alt="decentralizer" src="https://github.com/user-attachments/assets/27139bd2-452f-4bf4-b895-9c6d26a5c7fa" />

/
<img width="128" height="128" alt="DZR" src="https://github.com/user-attachments/assets/b11ef302-5179-463e-bedf-3c7ae35bfc5e" />
